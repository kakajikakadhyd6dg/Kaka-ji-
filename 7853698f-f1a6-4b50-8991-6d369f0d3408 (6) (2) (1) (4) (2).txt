...
# Upload Optimization Classes
class UploadProgress:
    def __init__(self, message, total_files):
    ...
    async def update_progress(self):
        ...
        text += f"▰{'▰' * int(progress/10)}{'▱' * (10 - int(progress/10))}\n"
        text += f"**{self.completed}/{self.total_files}** files uploaded\n"
    ...
    async def file_completed(self):
        self.completed += 1
        if self.completed % 2 == 0 or self.completed == self.total_files:
            await self.update_progress()

async def upload_with_semaphore(bot, document, caption, thumb=None, chat_id=None):
    """Upload files with semaphore to control concurrent uploads"""
    async with UPLOAD_SEMAPHORE:
        try:
            if document.endswith(('.mp4', '.mkv', '.avi', '.mov')):
                return await bot.send_video(
                    ...
                    progress=progress_bar if hasattr(progress_bar, '__call__') else None
                )
            elif document.endswith(('.jpg', '.jpeg', '.png')):
            ...
            return await upload_with_semaphore(bot, document, caption, thumb, chat_id)
        except Exception as e:
            logging.error(f"Upload error: {str(e)}")
            raise

async def compress_video_if_large(input_path, max_size_mb=MAX_FILE_SIZE):
    """Compress video if it's too large for faster upload"""
    ...
    if file_size > max_size_mb:
        ...
        compression_ratio = min(0.8, max_size_mb / file_size)
        crf = max(23, int(28 * compression_ratio))
        
        cmd = f'ffmpeg -i "{input_path}" -vcodec libx264 -crf {crf} -preset fast -movflags +faststart "{output_path}" -y'
        ...
        try:
            ...
            if os.path.exists(output_path) and os.path.getsize(output_path) > 0:
                ...
                logging.info(f"Compressed {file_size:.1f}MB -> {os.path.getsize(output_path)/(1024*1024):.1f}MB")
                return output_path
        except Exception as e:
...
async def batch_upload_files(bot, files_list, chat_id, progress_tracker=None):
    """Upload files in batches to avoid flooding and improve speed"""
    for i in range(0, len(files_list), BATCH_SIZE):
        batch = files_list[i:i + BATCH_SIZE]
        upload_tasks = []
        
        for file_info in batch:
            task = upload_with_semaphore(
                bot, 
                file_info['path'], 
                file_info['caption'],
                file_info.get('thumb'),
        ...
        # Upload batch concurrently with error handling
        ...
        for idx, result in enumerate(results):
            if not isinstance(result, Exception):
                if progress_tracker:
                    await progress_tracker.file_completed()
        ...
        # Brief pause between batches to avoid flooding
        if i + BATCH_SIZE < len(files_list):
            await asyncio.sleep(1)

async def download_video_optimized(url, cmd, name, max_retries=3):
    """Optimized download with retry mechanism and better error handling"""
    for attempt in range(max_retries):
        try:
            ...
            if process.returncode == 0:
                ...
                # Try with just .mp4 if no extension found
                if os.path.exists(f"{name}.mp4"):
                    return f"{name}.mp4"
                    
            else:
                error_msg = stderr.decode() if stderr else "Unknown error"
                logging.error(f"Download failed attempt {attempt + 1}: {error_msg}")
                
        except Exception as e:
            logging.error(f"Download error attempt {attempt + 1}: {str(e)}")
...
async def create_optimized_session():
    """Create optimized aiohttp session for better network performance"""
    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            if "youtube.com" in url or "youtu.be" in url:
                ...
                downloaded_file = await download_video_optimized(url, cmd, name)
                
                if downloaded_file and os.path.exists(downloaded_file):
                    try:
                        await upload_with_semaphore(
                            bot, 
                            downloaded_file, 
                            f'**🎵 Title : **  {name}.mp3\n\n🔗**Video link** : {url}\n\n🌟** Extracted By** : {CREDIT}',
                            chat_id=m.chat.id
...
async def txt_handler(bot: Client, m: Message):  
    ...
    try:
        ...
        logging.error(f"Pin message error: {str(e)}")
    
    # Initialize progress tracker and files list for batch upload
    progress_tracker = UploadProgress(await m.reply_text("🔄 Starting downloads..."), len(links) - int(raw_text) + 1)
    files_to_upload = []
    failed_count = 0
    count = int(raw_text)
    ...
    try:
        for i in range(count-1, len(links)):
            ...
            try:
                ...
                Show = f"<blockquote>🚀𝐏𝐫𝐨𝐠𝐫𝐞𝐬𝐬 » {progress:.2f}%</blockquote>\n┃\n" \
                       ...
                       f"━━━━━━━━━━━━━━━━━━━━━━━━━\n" \
                       f"<blockquote><b>⚡Dᴏᴡɴʟᴏᴀᴅɪɴɢ...⏳</b></blockquote>\n┃\n" \
                       f'┣💃𝐂𝐫𝐞𝐝𝐢𝐭 » {CR}\n┃\n' \
                       f"╰━📚𝐁𝐚𝐭𝐜𝐡 » {b_name}\n" \
                       f"━━━━━━━━━━━━━━━━━━━━━━━━━\n" \
                ...
                if downloaded_file and os.path.exists(downloaded_file):
                    # Compress if large
                    compressed_file = await compress_video_if_large(downloaded_file)
        ...
        # Batch upload all downloaded files
        if files_to_upload:
            await progress_tracker.message.edit_text("📤 Starting batch upload...")
            await batch_upload_files(bot, files_to_upload, m.chat.id, progress_tracker)

    except Exception as e:
        await m.reply_text(f"Error in main loop: {str(e)}")
...
async def text_handler(bot: Client, m: Message):
    ...
    try:
            ...
            # ... (same URL processing logic as above)
            ...
            if downloaded_file and os.path.exists(downloaded_file):
                ...
                await upload_with_semaphore(
                    bot,
                    compressed_file,
                    cc if downloaded_file.endswith(('.mp4', '.mkv')) else cc1,
                    thumb,
                    m.chat.id
                )
...
if __name__ == "__main__":
    print("🚀 Bot started with optimized upload speed!")
    bot.run()